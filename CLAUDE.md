...
- **Rankings** (pop, income, bolig, boligEnebolig, verne, energy, reservekapasitet, andelUtenLege, listelengde, gebyrTotal, grunnskolepoeng) are computed once at build time and stored per profile, not at runtime. `boligEnebolig` uses the 01→02→03 priority so the enebolig price shown in the snapshot stays consistent with its rank label.
